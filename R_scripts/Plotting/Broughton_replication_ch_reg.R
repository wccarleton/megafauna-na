...
for( n in 1:length(mfauna_sp)){
   load(paste("../Results/MCMC/Regional/mcmc_samples_",mfauna_sp[n],"_GL_ta_hc_BroughtonNGRIP_GL.RData",sep=""))
...
for( n in 1:length(mfauna_sp)){
   load(paste("../Results/MCMC/Regional/mcmc_samples_",mfauna_sp[n],"_SW_ta_hc_BroughtonNGRIP_SW.RData",sep=""))
   niter <- dim(samples$samples)[1]
   mcmc_samps <- as.data.frame(samples$samples[20000:niter,])
...
annotate_figure(fig,
                ...
                top=text_grob("Posterior Estimates (Humans and Climate)",face="bold",family="Times"))

ggsave(filename="../Images/posteriors_Broughton_ch_reg.png",
      device = "png",
      height = 20,
      width = 20,
      units = "cm",
