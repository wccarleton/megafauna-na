...
if(region == "GL"){
    load("../Data/RECE/rects_sample_arch_GL.RData")
}else{
    load("../Data/RECE/rects_sample_arch_SW.RData")
}
...
rece_sample_arch <- merge(data.frame(Date=NGRIP_d18O_ensemble$BP),get(paste("rects_sample_arch_",region,sep="")),all.x=T,by=1)
X2 <- as.matrix(rece_sample_arch[,sample(2:dim(get(paste("rects_sample_arch_",region,sep="")))[2],size=50)])
X2[which(is.na(X1))] <- 0
